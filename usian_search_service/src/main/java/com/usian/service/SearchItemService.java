...
public class SearchItemService {
    ...
    public Boolean importAll() {
        ...
        // 所以要分批量导入， 使用分页技术 一亿条数据  也可能是100万条数据，每条处理一毫秒， 大概需要半小时
        try {
            if(!isExistsIndex()){
                createIndex();
            }
            int page = 1;
            // 创建线程池， 伪代码， 线程吃要求池内有十个线程
            int total = 100;

            for(int i = 1 ; i < 101; i++){
                // 开启线程, 执行批量处理代码
                // 第一此循环，   i = 1， 执行批处理数据为 1 - 1000
                // 第二此循环，   i = 2， 执行批处理数据为 1001 - 2000
            }
            ...
            while (true){// 当使用线程池时，不能使用while true， 需要提前算好总页数。 总页数= 总记录数 ➗ 每页记录数，如果未整除则加一
                // page ++
                // 从线程池获取一个线程
